...
-- vim.opt.reletivenumber = true					# you can just jump with #D

-- Enable mouse mode, can be helpful for resizing splits for example
vim.opt.mouse = "a"

-- Don't show the mode, since its already in status line
...
-- Automatically recognize assembly script at the start and end of a file
-- vim.cmd [[ autocmd BufRead,BufNewFile *.asm,*.s set filetype=asm ]]
...
vim.keymap.set("n", "<left>", '<cmd>echo "Use h to move!!"<CR>') -- ✅ Correct
...
-- If lazy fim is not installed then it would install it
...
require("lazy").setup({
    ...
    -- Normally in NeoVim, you have to memorize keybindings.
    -- which-key.nvim shows a popup with available keybindings when you start typing.
    -- Example: If you press <leader>, a menu appears showing all keybindings that start with <leader>.
    -- IDK Consult an AI for this
    --
    ...
    --[[ 
  Lua LSP Plugin: `lazydev.nvim`
  
  - Enhances Lua LSP in NeoVim with better autocompletion and signatures.
  - Loads only for Lua files (`ft = 'lua'`) to optimize performance.
  - Provides Luvit (`vim.uv`) type definitions for improved API support.
  
  Usage:
  - Open a `.lua` file to get enhanced LSP features.
  - Requires an active Lua LSP (e.g., `lua-language-server`).
--]]
...

    {
        ...
        config = function()
            ...
            -- Since i am going to use C and python for nvim, I am going to uncoment the following lines to install the LSP for C and Python
            local servers = {
                clangd = {},
                -- gopls = {},
                pyright = {},
    ...
    -- Cool Neo Vim now auto formats my code
